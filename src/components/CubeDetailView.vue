<template>
    <div class="cube-detail-view">
        <div v-if="activeCube" class="cube-detail-header">
            <el-image :src="activeCube.thumbnail" fit="contain" class="cube-detail-image" />
            <div class="cube-detail-title-block">
                <el-link :href="`https://cubecobra.com/cube/about/${externalCubeId(activeCube)}`" target="_blank" type="default" underline="never">
                    <span class="cube-detail-name">{{ displayName(activeCube) }}</span>
                ...
                <el-link :href="`https://cubecobra.com/user/view/${activeCube.ownerId}`" target="_blank" underline="never">
                    <span class="cube-detail-owner">{{ activeCube.owner }}</span>
                </el-link>
                <div v-if="isSnapshot(activeCube)" class="cube-detail-snapshot-meta">
                    <el-icon><Clock /></el-icon>
                    <span>Snapshot · {{ snapshotDateLabel(activeCube.snapshotDate!) }}</span>
                    <template v-if="currentCubeLoaded">
                        <span class="cube-detail-snapshot-sep">·</span>
            ...
            <el-tooltip v-if="context === 'dialog'" content="Open in Cube tab" placement="top" :hide-after="50" :enterable="false">
                <el-button
                    :icon="TopRight"
                    size="small"
                    text
                    @click="openInCubeTab?.({ cubeId: activeCube!.id, subtab: activeTab, query: sharedSearchQuery })"
                    class="cube-detail-open-in-tab"
                />
            </el-tooltip>
        </div>

        <template v-if="activeCube">
            <div class="cube-detail-toolbar">
                <div v-if="!isSearchableTab" class="cube-detail-meta">
                    <span class="cube-detail-meta-item">Cards: <strong>{{ activeCube.stats?.totalCards ?? 0 }}</strong></span>
                    <span class="cube-detail-meta-item">Followers: <strong>{{ activeCube.followerCount ?? 0 }}</strong></span>
                    <el-tooltip :content="fullLastModified" placement="top" :hide-after="50" :enterable="false" :disabled="!fullLastModified">
                        <span class="cube-detail-meta-item">Modified: <strong>{{ formattedLastModified }}</strong></span>
                    </el-tooltip>
                    <span class="cube-detail-meta-item" v-if="(activeCube.stats?.assumedCategories || []).length">
                        Categories:
                        <el-tooltip
                ...
                <el-tab-pane label="Details" name="details">
                    <el-row class="details-tab">
                        <el-col v-if="activeCube?.brief" :span="24">
                            ...
                            <div class="cube-detail-brief" v-html="renderedBrief"></div>
                        </el-col>
                        <el-col :span="24">
        ...
        <div v-if="activeCube && !isMobile" class="cube-detail-footer">
            ...
            <el-button v-if="context === 'dialog'" @click="emit('close')">Close</el-button>
        </div>
    </div>
</template>

<script setup lang="ts">
import { ref, computed, watch, inject, provide, toRef, type PropType, type Ref } from 'vue';
import { useDateFormat, useWindowSize } from '@vueuse/core';
import { Loading, Link, Refresh, Clock, InfoFilled, Delete, CircleCheck, CircleClose, BrushFilled, Hide, TopRight } from '@element-plus/icons-vue';
...
const props = defineProps({
    ...
    context: {
        type: String as PropType<'dialog' | 'tab'>,
        default: 'dialog',
    },
});
...
const emit = defineEmits<{
    ...
    (e: 'select-cube', cubeId: string): void
    (e: 'close'): void
}>();

// Locally-scoped override: descendants (e.g. SimilarCubesTable) route
// cube-to-cube navigation through our emit instead of the App-level provide.
provide(openCubeDetailDialogKey, (id: string) => emit('select-cube', id));

const { width: windowWidth } = useWindowSize();
const isMobile = computed(() => windowWidth.value <= 760);

const openCardDetailDialog = inject(openCardDetailDialogKey);
const openInCubeTab = inject<((opts: { cubeId: string; subtab: string; query: string }) => void) | undefined>('openInCubeTab');
...
const sharedSearchQuery = computed({
...
const openCurrentCube = () => {
    if (currentCubeLoaded.value) {
        emit('select-cube', baseCubeId.value);
    }
};
...
// Refresh whenever the view switches to a different cube
...
// Reset activeCubeId whenever the view switches to a new cube
...
// Reset the shared search when the active cube changes.
watch(activeCubeId, () => {
...
.cube-detail-snapshot-sep {
    color: var(--el-text-color-disabled);
}

.cube-detail-open-in-tab {
    margin-left: auto;
}
...
.cube-detail-meta-item {
    display: flex;
    align-items: center;
    gap: 4px;
}

.cube-detail-meta-item strong {
